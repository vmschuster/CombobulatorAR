...
//  FIX THIS FOR LOADING FROM MAPS
  ...
  // tap handler for adding shapes
    @IBAction func handleTap(_ sender: UITapGestureRecognizer) {
      ...
      alert.addAction(UIAlertAction(title: "Save Object",
      ...
      alert.addAction(UIAlertAction(title: "Cancel Adding Object",
                                    style: UIAlertActionStyle.default,
                                    handler: nil))
      alert.addTextField(configurationHandler: {(objectNameFieldInput: UITextField!) in objectNameFieldInput.placeholder = "Enter Object Name"
          objectNameField = objectNameFieldInput
      })
  ...
  @IBAction func saveMap(_ sender: Any) {
    ...
    var inputMapName: UITextField?
    
    let alert = UIAlertController(title: "Save New Map", message: "Name Map", preferredStyle: UIAlertControllerStyle.alert)
    ...
    alert.addAction(UIAlertAction(title: "Save",
                                  style: UIAlertActionStyle.default,
                                  handler: { (alert: UIAlertAction!) -> Void in
                                    newMapName = String(inputMapName?.text ?? RandomName.Get())
                                    self.proceedToSave(savedMapName: newMapName)
                              }))
    alert.addTextField(configurationHandler: {(mapNameField: UITextField!) in
      mapNameField.placeholder = "Enter Map Name"
      inputMapName = mapNameField
    })
    self.present(alert, animated: true, completion: nil)
  }
    ...
    LibPlacenote.instance.loadMap(mapId: mapId,
      downloadProgressCb: {(completed: Bool, faulted: Bool, percentage: Float) -> Void in
        if (completed) {
          ...
          self.mapName = self.maps[indexPath.row].0
